...
int read_binary(void)
{
  ...
  nread = fread(&GV.L, sizeof(double), 1, inFile);
  nread = fread(&GV.NpTot, sizeof(long int), 1, inFile);
  nread = fread(&GV.OmegaM0, sizeof(double), 1, inFile);
  nread = fread(&GV.OmegaL0, sizeof(double), 1, inFile);
  nread = fread(&GV.zRS, sizeof(double), 1, inFile);
  nread = fread(&GV.HubbleParam, sizeof(double), 1, inFile);
  ...
  for(i=0; i<GV.NpTot; i++ )
    {
      ...
      part[i].pos[0] = pos_aux[0];
      part[i].pos[1] = pos_aux[1];
      part[i].pos[2] = pos_aux[2];
...
int write_binary(void)
{
  ...
  fwrite(&GV.OmegaL0, sizeof(double), 1, outFile);
...
# 79 "CIC.c"
  ...
  GV.rhoMean = totalMass / pow(GV.L, 3.0);
  ...
  for(i=0; i<GV.NpTot; i++)
    {
      locateCell(part[i].pos[0], part[i].pos[1], part[i].pos[2], i, cells);
    }
      ...
      for(j=0; j<GV.NGRID; j++)
 {
       ...
       for(l=0; l<Np; l++)
  {
    ...
    xp = part[idPart].pos[0];
    yp = part[idPart].pos[1];
    zp = part[idPart].pos[2];
     ...
     for(kk=-1; kk<=1; kk++)
       {
         ...
         indexaux = (mod(k+kk,GV.NGRID))+GV.NGRID*((mod(j+jj,GV.NGRID))+GV.NGRID*(mod(i+ii,GV.NGRID)));
         xc = GV.dx*(0.5 + i+ii);
         yc = GV.dx*(0.5 + j+jj);
         zc = GV.dx*(0.5 + k+kk);
   ...
   for(k=0; k<GV.NGRID; k++)
     {
       ...
       cells[index].pos[0] = GV.dx * (0.5 + i);
       cells[index].pos[1] = GV.dx * (0.5 + j);
